    ...
    companion object {
        ...
        private val MIGRATION_1_2 = object : Migration(1, 2) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE transactions ADD COLUMN category TEXT NOT NULL DEFAULT 'all'")
        ...
        private val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "UPDATE transactions SET category = 'all' WHERE category IS NULL OR category = ''"
                )
            }
        }
        ...
        private val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL(
                    "INSERT INTO transactions_new (id, amount, type, date, goal_id, bill_id, account_id, note, category) " +
                        "SELECT id, amount, type, date, goal_id, bill_id, account_id, note, category FROM transactions"
                )
                db.execSQL("DROP TABLE transactions")
                db.execSQL("ALTER TABLE transactions_new RENAME TO transactions")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_transactions_date ON transactions(date)")
